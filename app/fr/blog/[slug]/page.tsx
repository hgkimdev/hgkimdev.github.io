...
export async function generateMetadata({
  ...
  const post = getAllPosts("fr").find((p) => p.slug === slug);
  return {
    title: post ? `${post.title} · Blog` : "Blog",
    description: post?.summary || undefined,
    alternates: post
      ? { canonical: absoluteUrl(localizeHref(`/blog/${slug}`, "fr")) }
      : undefined,
  };
}
...
export default async function BlogPostPage({ params }: BlogPostPageProps) {
  ...
  const html = await getPostHtml(slug, "fr");

  return (
    <BlogPostView post={post} html={html} allPosts={posts} locale="fr" />
  );
}
