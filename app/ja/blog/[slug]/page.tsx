...
export async function generateMetadata({
  ...
  const post = getAllPosts("ja").find((p) => p.slug === slug);
  return {
    title: post ? `${post.title} · Blog` : "Blog",
    description: post?.summary || undefined,
    alternates: post
      ? { canonical: absoluteUrl(localizeHref(`/blog/${slug}`, "ja")) }
      : undefined,
  };
}
...
export default async function BlogPostPage({ params }: BlogPostPageProps) {
  ...
  const html = await getPostHtml(slug, "ja");

  return (
    <BlogPostView post={post} html={html} allPosts={posts} locale="ja" />
  );
}
