...
- **사람 눈에 보이는 링크는 두 곳**: 블로그 사이드바의 분류 축 꼬리(`blog-sidebar.tsx`)와 푸터의 연락처 아이콘 줄(`site-footer.tsx`). **사이드바 쪽은 900px 이상에서만 보인다** — 좁은 화면에서 사이드바는 가로로 스크롤하는 칩 한 줄이 되는데, 거기 한 줄을 더 만들면 높이를 항목 수와 무관하게 유지하려고 가로 스크롤을 고른 이유가 무너진다. 그래서 모바일에서 피드로 가는 길은 푸터 아이콘 하나다. 둘 다 `next/link`가 아니라 `<a>`다 — `/feed.xml`은 라우트가 아니라 정적 파일이라 클라이언트 내비게이션의 대상이 아니다. 푸터는 Blog 존뿐 아니라 `/projects/[slug]`에도 붙으므로 RSS 아이콘이 거기에도 나온다(사이트 공용 푸터의 공용 링크로 본다). `aria-label`은 사전으로 빼지 않았다: RSS는 번역되는 낱말이 아니라 형식 이름이고 두 로케일에서 같은 글자다
