// 콘텐츠 상태
//
// 전부 실제 내용이다: 카테고리·항목 구성, 제목, meta(연도·저자·시기),
// 모든 항목의 `why`, 여행의 `wish` 목록까지 본인이 확인한 문장이다.
// YouTube ID는 전부 공식 채널 영상으로 확인해 넣었다.
//
// 영어 번역은 content/life.en.ts에 항목 id로 매칭해 따로 둔다 — media·id
// 등 언어 무관 필드를 중복시키지 않기 위해서다. lib/content/life.ts의
// getLifeCategories(locale)가 합쳐서 돌려준다.
//
// 배경 그림: 영화·게임은 예고편, 책·여행은 각각 표지·갤러리 사진이 있다.
// 언어는 영어만 책 표지를 재활용해 채웠다 — 프랑스어(듀오링고 커스텀
// 그래픽)는 시도했다가 어울리지 않아 되돌렸고, 일본어·이탈리아어는 아직
// 채울 그림이 없다 — 셋 다 `kind: "none"`으로 남아 있어 입구 액자와
// 오버레이 배경이 비어 있다.
//
 ...
 * cefr이 null이면 레벨 미표기다 — 점 여섯 개가 전부 빈 채로 그려진다.
 */
  ...
  },
  {
    ...
    items: [
      ...
      },
      {
        id: "italian",
        title: "이탈리아어",
        // 레벨도 상태도 표기하지 않는다. 그래도 다른 언어와 같은 자리에 같은
        // 배지를 두되 안을 비운다(status "", cefr null) — 이 항목만 배지가
        // 빠지면 목록에서 줄이 어긋나 보인다. 어떤 언어인지는 아래 why가
        // 말한다. meta는 level이 있으면 그려지지 않으므로 같이 비워둔다.
        meta: "",
        level: { status: "", cefr: null },
        why: [
          "취미로 몇 달 가볍게 독학했다.",
          "지금은 하지 않고,",
          "다시 할 계획도 아직 없다.",
