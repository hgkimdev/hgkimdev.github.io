...
/**
 * About 슬롯의 배경 — 이 사이트의 소스가 화면 뒤로 계속 흘러 올라간다.
 *
 * **왜 흐르나.** Life의 사진 벽과 같은 이유다(life-entrance.tsx의 FlowingWall
 * 주석). 멈춰 있으면 "코드 스크린샷 한 장"으로 읽히고, 천천히 흐르면 화면
 * 밖에도 계속 이어진다는 뜻이 된다.
 *
 * **방향은 거기와 반대인 위쪽이다.** 사진은 순서가 없어서 어느 쪽이든 됐지만
 * 코드에는 읽는 순서가 들어 있다 — 아래로 흘리면 파일을 거꾸로 훑게 된다
 * (자세한 건 globals.css의 code-backdrop-flow 주석). 한때 여기에 "두 섹션이
 * 한 페이지에서 반대로 흐르면 헷갈린다"고 적혀 있었는데, 슬롯 순서가
 * about → projects → life라 두 배경은 애초에 같이 화면에 있지 않는다.
 *
 ...
 * 흐름이 26.7px/s(한 줄이 지나가는 데 0.92초)라 잠깐 머무는 사람에게는 사실상
 * 정지된 한 화면이다. 이 속도가 어디서 나온 값인지는 globals.css의
 * code-backdrop-track 주석에 있다 — 느낌이 아니라 픽셀 격자에서 나왔다. 원본
